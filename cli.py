...
from engine.report_generator import display

console = Console()

def main():
    parser = argparse.ArgumentParser(description="电商代运营多店铺经营分析与白牌周报自动化引擎")
    parser.add_argument("--orders", default="tests/fixtures/orders_export.csv", help="订单明细导出表路径 (CSV)")
    parser.add_argument("--ads", default="tests/fixtures/ads_spend_export.csv", help="推广消耗导出表路径 (CSV)")
    parser.add_argument("--inventory", default="tests/fixtures/inventory_cost.csv", help="商品成本库存表路径 (CSV)")
    ...
    console.print(Panel.fit(
        ...
        f"[dim]白牌代工模式 · 确定性会计级勾稽自检 · 本地生成，发送前需审核[/dim]",
        border_style="cyan"
    ))
    ...
    table.add_row("总成交额 (GMV)", f"¥{display(summary['total_gmv'], ",.2f")} ", f"有效支付订单 {summary['total_orders']} 笔")
    table.add_row("净销售额 (Net)", f"¥{display(summary['total_net_sales'], ",.2f")} ", "已剔除全部售后退款")
    refund_style = "red" if summary["refund_rate_pct"] > 15 else "yellow"
    table.add_row("退款总额 / 退款率", f"¥{display(summary['total_refunds'], ",.2f")} ", f"[{refund_style}]{display(summary['refund_rate_pct'], ".1f")} %[/{refund_style}]")
    table.add_row("推广总消耗 (Ads)", f"¥{display(summary['total_ad_spend'], ",.2f")} ", "全渠道付费引流总计")
    roi_style = "green" if summary["blended_roi"] is not None and summary["blended_roi"] >= 3.5 else "yellow"
    table.add_row("全店综合投产比 (ROI)", f"[{roi_style}]{display(summary['blended_roi'], ".2f")} [/{roi_style}]", "GMV / 推广消耗")
    table.add_row("广告后贡献利润", f"¥{display(summary['gross_profit'], ",.2f")} ", f"贡献利润率 [bold green]{display(summary['gross_profit_margin_pct'], ".1f")} %[/bold green]")
    ...
    console.print(Panel(
        ...
        title="[bold green]本地生成完成，未发送[/bold green]",
        border_style="green"
    ))
